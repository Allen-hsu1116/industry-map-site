...
function SearchIcon() {
  return <svg className="w-4.5 h-4.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>;
}
function CloseIcon() {
...
export default function Home() {
  ...
  const goToCompany = (code: string) => {
    ...
    // If already in companies tab, just update. Otherwise show modal.
  ...
  return (
    <div className="min-h-screen bg-[#080c18] text-[#e2e8f0] flex flex-col">
        ...
        <div className="max-w-[1400px] mx-auto px-8 lg:px-12">
          <div className="flex items-center h-16 gap-8">
            ...
            <div ref={searchRef} className="relative flex-1 max-w-xl mx-auto">
              <div className={`flex items-center rounded-2xl transition-all duration-200 ${searchFocused ? "bg-[#151c2e] ring-2 ring-indigo-500/30" : "bg-[#111827]"}`}>
                <input type="text" placeholder="搜尋題材、公司名稱或代碼..." className="w-full bg-transparent px-6 py-2.5 text-sm text-white placeholder-[#4a5568] outline-none" value={search} onChange={(e) => { setSearch(e.target.value); setShowAutocomplete(true); }} onFocus={() => { setShowAutocomplete(true); setSearchFocused(true); }} onBlur={() => setSearchFocused(false)} />
          ...
          <div className="flex items-center gap-1.5 -mb-px overflow-x-auto pb-0.5">
            {([
              ...
              <button key={tab.id} className={`nav-tab px-6 py-3.5 text-sm font-medium whitespace-nowrap transition-all ${activeTab === tab.id ? "active text-indigo-400" : "text-[#546280] hover:text-[#8b9ab8]"}`} onClick={() => setActiveTab(tab.id)}>
      ...
      <div className="relative bg-[#080c18] border-b border-white/[0.06]">
        <div className="max-w-[1400px] mx-auto px-8 lg:px-12 py-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[
              { emoji: "📋", number: stats.total_topics, label: "題材數", color: "text-indigo-400", bg: "bg-indigo-500" },
              { emoji: "🏢", number: stats.unique_companies, label: "不重複公司", color: "text-emerald-400", bg: "bg-emerald-500" },
              { emoji: "📊", number: stats.total_companies, label: "公司條目", color: "text-amber-400", bg: "bg-amber-500" },
              { emoji: "🏷️", number: categories.length - 1, label: "產業類別", color: "text-rose-400", bg: "bg-rose-500" },
            ].map((stat, i) => (
              <div key={i} className="stats-card rounded-2xl p-6 flex items-center gap-5">
                <div className={`w-12 h-12 rounded-xl ${stat.bg}/10 flex items-center justify-center text-xl`}>{stat.emoji}</div>
                <div><div className={`text-2xl font-bold ${stat.color} leading-none`}>{stat.number}</div><div className="text-xs text-[#546280] mt-1.5">{stat.label}</div></div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ─── Main Content ─── */}
      <main className="flex-1 relative max-w-[1400px] mx-auto w-full px-8 lg:px-12 py-10">

        {/* ─── Focus Tab ─── */}
        {activeTab === "focus" && (
          <div className="fade-in">
            <div className="bg-[#111827] rounded-3xl border border-white/[0.06] p-16 text-center max-w-2xl mx-auto mt-8">
              ...
              <div className="mt-10 flex justify-center gap-4">
                <button className="px-8 py-3.5 rounded-2xl bg-gradient-to-r from-indigo-500 to-purple-600 text-white text-sm font-semibold hover:from-indigo-600 hover:to-purple-700 transition-all shadow-lg shadow-indigo-500/25" onClick={() => setActiveTab("topics")}>瀏覽全部題材</button>
                <button className="px-8 py-3.5 rounded-2xl bg-white/[0.05] border border-white/[0.08] text-white text-sm font-medium hover:bg-white/[0.08] transition-all" onClick={() => setActiveTab("map")}>產業地圖</button>
              </div>
            </div>
          </div>
        )}
            ...
            <aside className="hidden lg:block w-56 shrink-0">
              <div className="sticky top-[140px] space-y-2">
                ...
                {categories.map((cat) => {
                  ...
                  return (
                    <button key={cat} className={`sidebar-category w-full flex items-center justify-between px-4 py-2.5 rounded-xl text-sm transition-all ${selectedCategory === cat ? "active bg-indigo-500/10 text-white" : "text-[#8b9ab8] hover:text-white hover:bg-white/[0.03]"}`} onClick={() => setSelectedCategory(cat)}>
                      <div className="flex items-center gap-2.5">{cat !== "全部" && <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: color.solid }} />}<span className="truncate">{cat}</span></div>
                      <span className={`text-xs px-2.5 py-0.5 rounded-lg font-medium ${selectedCategory === cat ? "bg-indigo-500/20 text-indigo-300" : "bg-white/[0.04] text-[#546280]"}`}>{count}</span>
                    </button>
                  );
                })}
              </div>
            ...
            <div className="flex-1 min-w-0">
              <div className="lg:hidden flex flex-wrap gap-2.5 mb-6 overflow-x-auto pb-2">
                {categories.map((cat) => {
                  const color = cat === "全部" ? DEFAULT_COLOR : CATEGORY_COLORS[cat] || DEFAULT_COLOR;
                  return (
                    <button key={cat} className={`category-pill px-4 py-2 rounded-xl text-xs font-medium whitespace-nowrap border transition-all ${selectedCategory === cat ? "bg-indigo-500/15 border-indigo-500/40 text-indigo-300" : "bg-[#111827] border-white/[0.06] text-[#8b9ab8] hover:border-white/[0.12]"}`} onClick={() => setSelectedCategory(cat)}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {filteredTopics.map((topic) => {
                  ...
                  return (
                    <button key={topic.slug} className="topic-card-item text-left rounded-2xl border border-white/[0.06] overflow-hidden transition-all hover:border-white/[0.12] hover:shadow-xl hover:shadow-indigo-500/5" onClick={() => goToTopic(topic.slug)}>
                      <div className={`h-1.5 bg-gradient-to-r ${color.gradient}`} />
                      <div className="bg-[#111827] px-7 py-6">
                        <div className="flex items-start justify-between gap-3 mb-3">
                          <div className="flex items-center gap-2.5 min-w-0"><span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: color.solid }} /><span className="text-[11px] font-medium text-[#546280] uppercase tracking-wider truncate">{cat}</span></div>
                        ...
                        <h3 className="font-semibold text-[15px] text-white leading-relaxed mb-2.5 line-clamp-2">{topic.name}</h3>
                        {topic.description && (<p className="text-xs text-[#546280] line-clamp-2 leading-relaxed">{topic.description.substring(0, 120)}{topic.description.length > 120 ? "..." : ""}</p>)}
              ...
              {filteredTopics.length === 0 && (<div className="text-center py-24"><div className="text-5xl mb-4">🔍</div><p className="text-[#546280] text-lg">找不到符合條件的題材</p></div>)}
            </div>
          </div>
        )}
          ...
          <div className="fade-in">
            {!selectedTopicData ? (
              <div className="text-center py-24">
                ...
                <button className="px-8 py-3.5 rounded-2xl bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-medium hover:from-indigo-600 hover:to-purple-700 transition-all shadow-lg shadow-indigo-500/25" onClick={() => setActiveTab("topics")}>📋 瀏覽題材總覽</button>
                <div className="mt-12 max-w-3xl mx-auto">
                  <h3 className="text-sm font-medium text-[#546280] mb-5">熱門題材</h3>
                  <div className="flex flex-wrap gap-3 justify-center">{topics.slice(0, 12).map((t) => { const cat = getCategory(t.name); const color = CATEGORY_COLORS[cat] || DEFAULT_COLOR; return (<button key={t.slug} className="px-5 py-2.5 rounded-xl bg-[#111827] border border-white/[0.06] text-sm text-[#8b9ab8] hover:text-white hover:border-indigo-500/40 transition-all" onClick={() => goToTopic(t.slug)}><span className="inline-block w-2 h-2 rounded-full mr-2" style={{ backgroundColor: color.solid }} />{t.name}</button>); })}</div>
                </div>
              </div>
            ) : (
              <div className="flex gap-12">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-4 mb-8">
                    <button className="w-10 h-10 rounded-xl bg-[#111827] border border-white/[0.06] flex items-center justify-center text-[#8b9ab8] hover:text-white hover:border-white/[0.12] transition-colors" onClick={() => setSelectedTopicSlug(null)}><ArrowIcon /></button>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2.5 mb-1.5">{(() => { const cat = getCategory(selectedTopicData.name); const color = CATEGORY_COLORS[cat] || DEFAULT_COLOR; return <><span className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: color.solid }} /><span className="text-xs font-medium text-[#546280]">{cat}</span></>; })()}</div>
                  ...
                  {selectedTopicData.description && (<div className="bg-[#111827] rounded-2xl border border-white/[0.06] p-7 mb-7"><p className="text-sm text-[#8b9ab8] leading-[1.8]">{selectedTopicData.description}</p></div>)}
                    ...
                    <div className="space-y-5">
                      {mapGroupNames(selectedTopicData.groups).map((group, gi) => (
                        <div key={gi} className="bg-[#111827] rounded-2xl border border-white/[0.06] overflow-hidden">
                          <div className="px-7 py-5 border-b border-white/[0.06] flex items-center justify-between"><h3 className="font-semibold text-sm text-white">{group.name}</h3><span className="text-xs text-[#546280]">{group.companies.length} 家公司</span></div>
                          <div className="divide-y divide-white/[0.04]">
                            {group.companies.map((company) => { const relInfo = getRelevanceInfo(company.relevance); return (
                              <button key={company.code} className="company-card w-full flex items-center justify-between px-7 py-5 gap-4 text-left" onClick={() => goToCompany(company.code)}>
                                <div className="flex items-center gap-4 min-w-0">
                                  <div className="w-11 h-11 rounded-xl bg-[#0a0e1a] border border-white/[0.06] flex items-center justify-center shrink-0"><span className="text-xs font-mono font-bold text-[#8b9ab8]">{company.code.slice(0, 4)}</span></div>
                                  <div className="min-w-0"><div className="flex items-center gap-2"><span className="text-sm font-medium text-white">{company.name}</span><span className="text-xs text-[#546280]">{company.code}</span></div>{company.role && <p className="text-xs text-[#546280] truncate mt-0.5">{company.role}</p>}</div>
                                </div>
                        ...
                        return namedGroups.map((group, gi) => {
                          ...
                          return (
                            <div key={gi} className="slide-up" style={{ animationDelay: `${gi * 80}ms` }}>
                              <div className="flex items-center gap-2.5 mb-5"><span className="text-lg">{level.icon}</span><h3 className={`font-bold text-sm ${level.text}`}>{level.label}：{group.name}</h3><span className="text-xs text-[#546280] ml-auto">{group.companies.length} 家</span></div>
                              {gi > 0 && (<div className="flex justify-center my-5"><div className="flex flex-col items-center"><div className="w-0.5 h-8 bg-gradient-to-b from-white/[0.04] to-white/[0.12]" /><svg className="w-4 h-4 text-[#546280] -mt-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 14l-7 7m0 0l-7-7m7 7V3" /></svg></div></div>)}
                              <div className={`${level.bg} ${level.border} border rounded-2xl p-6`}>
                                <div className="flex flex-wrap gap-3">
                                  {group.companies.map((company) => { const relInfo = getRelevanceInfo(company.relevance); return (
                                    <button key={company.code} className="bg-[#0a0e1a]/80 border border-white/[0.06] rounded-xl px-5 py-3 flex items-center gap-2.5 hover:border-indigo-500/30 hover:bg-[#0a0e1a] transition-all cursor-pointer" onClick={() => goToCompany(company.code)}>
                                      <div className="text-center"><div className="text-xs font-bold text-white">{company.code}</div><div className="text-[11px] text-[#8b9ab8]">{company.name}</div></div>
                ...
                <div className="hidden xl:block w-72 shrink-0">
                  <div className="sticky top-[140px] space-y-6">
                    <div className="bg-[#111827] rounded-2xl border border-white/[0.06] p-6"><h4 className="text-[11px] font-bold text-[#546280] uppercase tracking-widest mb-5">題材概要</h4><div className="space-y-4"><div className="flex justify-between text-sm"><span className="text-[#8b9ab8]">公司總數</span><span className="text-white font-medium">{selectedTopicData.total}</span></div><div className="flex justify-between text-sm"><span className="text-[#8b9ab8]">供應鏈層級</span><span className="text-white font-medium">{selectedTopicData.groups.length}</span></div>{(() => { const highRel = selectedTopicData.groups.reduce((acc, g) => acc + g.companies.filter((c) => String(c.relevance) === "高" || ["80","85","90","95"].includes(String(c.relevance))).length, 0); return <div className="flex justify-between text-sm"><span className="text-[#8b9ab8]">核心公司</span><span className="text-emerald-400 font-medium">{highRel}</span></div>; })()}</div></div>
                    <div className="bg-[#111827] rounded-2xl border border-white/[0.06] p-6"><h4 className="text-[11px] font-bold text-[#546280] uppercase tracking-widest mb-5">同類題材</h4><div className="space-y-2">{topics.filter((t) => getCategory(t.name) === getCategory(selectedTopicData.name) && t.slug !== selectedTopicData.slug).slice(0, 5).map((t) => (<button key={t.slug} className="w-full text-left px-4 py-2.5 rounded-xl text-sm text-[#8b9ab8] hover:text-white hover:bg-white/[0.04] transition-colors truncate" onClick={() => goToTopic(t.slug)}>{t.name}</button>))}</div></div>
                  </div>
                </div>
              </div>
            )}
        ...
        {activeTab === "companies" && (
          <div className="fade-in">
            <div className="flex items-center gap-6 mb-8">
              <div className="relative flex-1 max-w-xl"><div className="flex items-center rounded-2xl bg-[#111827]"><input type="text" placeholder="搜尋公司名稱或代碼..." className="w-full bg-transparent px-6 py-3 text-sm text-white placeholder-[#4a5568] outline-none" value={companySearch} onChange={(e) => setCompanySearch(e.target.value)} /><div className="pr-5 text-[#4a5568]"><SearchIcon /></div></div></div>
            ...
            <div className="flex gap-8">
              <div className={`transition-all duration-300 ${selectedCompanyData ? "w-[45%]" : "w-full"}`}>
                <div className="bg-[#111827] rounded-2xl border border-white/[0.06] overflow-hidden">
                  <div className="grid grid-cols-[100px_1fr_100px] px-7 py-4 border-b border-white/[0.06] bg-[#0a0e1a]/60"><span className="text-[11px] font-bold text-[#546280] uppercase tracking-wider">代碼</span><span className="text-[11px] font-bold text-[#546280] uppercase tracking-wider">名稱</span><span className="text-[11px] font-bold text-[#546280] uppercase tracking-wider text-right">題材數</span></div>
                  <div className="divide-y divide-white/[0.04] max-h-[calc(100vh-280px)] overflow-y-auto">
                    {filteredCompanies.slice(0, 150).map((company) => (
                      <button key={company.code} className={`company-row w-full grid grid-cols-[100px_1fr_100px] px-7 py-4 items-center gap-4 text-left transition-colors ${selectedCompanyCode === company.code ? "bg-indigo-500/10" : "hover:bg-white/[0.03]"}`} onClick={() => { setSelectedCompanyCode(company.code); }}>
                        ...
                        <span className="text-sm text-right"><span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-indigo-500/15 text-xs text-indigo-300 font-bold">{company.topic_count}</span></span>
                      </button>
                    ))}
                  </div>
                  {filteredCompanies.length > 150 && (<div className="px-7 py-4 border-t border-white/[0.04] text-center text-xs text-[#546280]">顯示前 150 筆，共 {filteredCompanies.length} 家公司</div>)}
                </div>
              </div>
              {selectedCompanyData && (
                  ...
                  <div className="bg-[#111827] rounded-2xl border border-white/[0.06] p-8 sticky top-[140px]">
                    <div className="flex items-center justify-between mb-6">
                      <div className="flex items-center gap-4">
                        ...
                        <div><h2 className="text-xl font-bold text-white">{selectedCompanyData.name}</h2><span className="text-sm text-[#546280]">{selectedCompanyData.code}</span></div>
                      </div>
                      <button className="w-9 h-9 rounded-xl bg-white/[0.04] border border-white/[0.06] flex items-center justify-center text-[#546280] hover:text-white transition-colors" onClick={() => setSelectedCompanyCode(null)}><CloseIcon /></button>
                    </div>
                    <div className="grid grid-cols-2 gap-4 mb-7">
                      <div className="bg-[#0a0e1a] rounded-xl p-4 text-center"><div className="text-2xl font-bold text-indigo-400">{selectedCompanyData.topic_count}</div><div className="text-xs text-[#546280] mt-1">相關題材</div></div>
                      <div className="bg-[#0a0e1a] rounded-xl p-4 text-center"><div className="text-2xl font-bold text-emerald-400">{selectedCompanyData.roles.length}</div><div className="text-xs text-[#546280] mt-1">供應鏈角色</div></div>
                    </div>
                    <h3 className="text-[11px] font-bold text-[#546280] uppercase tracking-widest mb-4">供應鏈角色</h3>
                    <div className="space-y-3 mb-7">
                      {selectedCompanyData.roles.map((role, i) => { const relInfo = getRelevanceInfo(role.relevance); return (
                        <div key={i} className="bg-[#0a0e1a] rounded-xl p-4">
                          <div className="flex items-center justify-between mb-1.5"><button className="text-sm text-indigo-400 hover:text-indigo-300 font-medium text-left" onClick={() => goToTopic(role.topic)}>{role.topicName}</button><span className={`${relInfo.className} text-xs px-2.5 py-0.5 rounded-full font-medium`}>{relInfo.emoji} {relInfo.label}</span></div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedCompanyData.relatedTopics.map((t) => { const cat = getCategory(t.name); const color = CATEGORY_COLORS[cat] || DEFAULT_COLOR; return (<button key={t.slug} className="px-3 py-1.5 rounded-lg text-xs font-medium border hover:border-indigo-500/40 transition-colors" style={{ backgroundColor: `${color.solid}12`, borderColor: `${color.solid}30`, color: color.solid }} onClick={() => goToTopic(t.slug)}>{t.name}</button>); })}
                    </div>
                  </div>
                </div>
              )}
      ...
      {showCompanyModal && selectedCompanyData && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center" onClick={() => setShowCompanyModal(false)}>
          ...
          <div className="relative bg-[#111827] rounded-2xl border border-white/[0.1] p-8 max-w-lg w-full mx-4 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-lg font-bold text-white shadow-lg shadow-indigo-500/20">{selectedCompanyData.code.slice(0, 4)}</div>
                <div><h2 className="text-xl font-bold text-white">{selectedCompanyData.name}</h2><span className="text-sm text-[#546280]">{selectedCompanyData.code}</span></div>
              </div>
              <button className="w-9 h-9 rounded-xl bg-white/[0.04] border border-white/[0.06] flex items-center justify-center text-[#546280] hover:text-white transition-colors" onClick={() => setShowCompanyModal(false)}><CloseIcon /></button>
            </div>
            ...
            <div className="space-y-2 mb-6 max-h-60 overflow-y-auto">
              {selectedCompanyData.roles.slice(0, 10).map((role, i) => { const relInfo = getRelevanceInfo(role.relevance); return (
                <div key={i} className="bg-[#0a0e1a] rounded-lg px-4 py-3 flex items-center justify-between">
                  <button className="text-sm text-indigo-400 hover:text-indigo-300 font-medium text-left truncate" onClick={() => { setShowCompanyModal(false); goToTopic(role.topic); }}>{role.topicName}</button>
                  <div className="flex items-center gap-2 shrink-0"><span className="text-xs text-[#8b9ab8]">{role.group}</span><span className={`${relInfo.className} text-[11px] px-2 py-0.5 rounded-full font-medium`}>{relInfo.label}</span></div>
                </div>
              ); })}
            </div>
            <div className="flex gap-3">
              <button className="flex-1 px-5 py-3 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-600 text-white text-sm font-medium hover:from-indigo-600 hover:to-purple-700 transition-all" onClick={() => { setShowCompanyModal(false); setSelectedCompanyCode(selectedCompanyData.code); setActiveTab("companies"); }}>查看完整資料</button>
              <button className="px-5 py-3 rounded-xl bg-white/[0.05] border border-white/[0.08] text-[#8b9ab8] text-sm font-medium hover:text-white transition-colors" onClick={() => setShowCompanyModal(false)}>關閉</button>
            </div>
          </div>
        </div>
      )}
