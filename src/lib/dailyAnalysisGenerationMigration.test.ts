...
test("package scripts expose a daily refresh workflow that updates K-lines, chip feeds, and event focus before regenerating analysis", async () => {
  ...
  assert.equal(
    ...
    "npm run data:kline:update -- --limit=30 && npm run data:market-feeds:update -- --limit=30 && npm run data:event-focus:update && npm run analysis:daily && npm run report:daily && npm run knowledge:validate",
  );
  assert.equal(
    packageJson.scripts["data:daily-refresh:dry-run"],
    "npm run data:kline:update -- --dry-run --limit=2 && npm run data:market-feeds:update -- --dry-run --limit=2 && npm run data:event-focus:update -- --dry-run --limit=10 && npm run report:daily && npm run knowledge:validate",
  );
  ...
  assert.ok(packageJson.scripts["data:daily-refresh"].indexOf("analysis:daily") < packageJson.scripts["data:daily-refresh"].indexOf("report:daily"));
